...
class AppImageFixer:
    ...
    def __init__(self, home_dir: Optional[Path] = None, log_file: Optional[Path] = None):
        """
    ...
    def extract_appimage_version(self, appimage_path: Path) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                version_match = re.search(r'version[:\s]+([0-9]+\.[0-9]+\.[0-9]+)', result.stdout, re.IGNORECASE)
                if version_match:
                    return version_match.group(1)
    ...
    def fix_icon_references(self, lines: List[str], icon_name: str) -> Tuple[List[str], bool]:
    ...
    def needs_fixing(self, file_path: Path, icon_name: str, needs_no_sandbox: bool) -> bool:
    ...
    def fix_desktop_file(self, file_path: Path) -> bool:
        ...
        # Create backup
        backup_path = file_path.with_suffix(file_path.suffix + FIXER_SETTINGS["backup_extension"])
        try:
    ...
    def find_appimage_files(self) -> List[Path]:
        ...
        if status['config_loaded'] or status['running']:
            ...
            self.log(f"Found {len(appimage_files)} AppImageLauncher desktop files via integration")
            return appimage_files
        else:
            # Fallback to default directory
            self.log("No AppImageLauncher/appimaged integration found, using default ~/Applications", "INFO")
    ...
    def find_appimage_executables(self) -> List[Path]:
        ...
        # Check if AppImageLauncher/appimaged integration is available
        if status['config_loaded'] or status['running']:
            self.log("Using AppImageLauncher/appimaged integration for executable search", "INFO")
            return integration.find_appimage_files()
        else:
            # Fallback to default directories
            self.log("No AppImageLauncher/appimaged integration found, using default directories", "INFO")
            default_dirs = [
                ...
                if directory.exists():
                    ...
                    for file_path in directory.rglob("*"):
                        if (file_path.is_file() and 
                            ...
                            self._is_appimage_file(file_path)):
                            appimage_files.append(file_path)
            
            self.log(f"Found {len(appimage_files)} AppImage files in default directories")
            return appimage_files
    ...
    def run(self) -> Dict[str, any]:
        ...
        self.log(f"Database updated: {db_result['updated']} records, cleaned: {db_result['cleaned']} orphaned")
