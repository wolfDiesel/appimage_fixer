    ...
    for app in appimages:
        ...
        if app['last_scan']:
            try:
                ...
                last_scan = app['last_scan'][:16] if len(app['last_scan']) >= 16 else app['last_scan'][:10]
            except:
                last_scan = app['last_scan'][:10]
        else:
            last_scan = "N/A"
        print(f"{app['name']:<25} {app['version'] or 'N/A':<15} {status:<15} {last_scan}")
...
def check_versions():
    ...
    print("=" * 80)
    print(f"{'Application':<25} {'Desktop Version':<15} {'AppImage Version':<15} {'Status'}")
    print("-" * 80)
...
def check_appimaged_integration():
    ...
    for appimage_path, desktop_path in mapping.items():
        ...
        print(f"  {status} {appimage_path.name} -> {desktop_path.name if desktop_path else 'No desktop file'}")
...
Examples:
  ...
  appimage-fixer install-service       # Install systemd service
    ...
    parser.add_argument(
        "--version", 
        action="version", 
        version=f"AppImage Fixer v{__version__}"
    )
    ...
    subparsers.add_parser("check-versions", help="Check version consistency between desktop files and AppImage files")
    
    # AppImageD integration command
    subparsers.add_parser("check-appimaged", help="Check AppImageD integration and show status")
    ...
    if args.dry_run:
        ...
        for file_path in desktop_files:
            ...
            if app_name:
                ...
                needs_fixing = fixer.needs_fixing(file_path, config["icon"], config["needs_no_sandbox"])
