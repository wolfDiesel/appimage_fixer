...
class AppImageDIntegration:
    ...
    def get_appimage_directories(self) -> List[Path]:
        ...
        if self.config.has_section("appimagelauncherd"):
            if self.config.has_option("appimagelauncherd", "additional_directories_to_watch"):
                additional_dirs = self.config.get("appimagelauncherd", "additional_directories_to_watch")
                if additional_dirs:
    ...
    def get_appimage_desktop_mapping(self) -> Dict[Path, Optional[Path]]:
        ...
        for appimage_path in appimage_files:
            ...
            for desktop_path in desktop_files:
                ...
                if (app_name.lower() in desktop_name.lower() or 
                    desktop_name.lower() in app_name.lower()):
    ...
    def get_appimaged_status(self) -> Dict[str, any]:
        ...
        # Get directories
        status["appimage_directories"] = [str(d) for d in self.get_appimage_directories()]
        status["desktop_directories"] = [str(d) for d in self.get_desktop_file_directories()]
