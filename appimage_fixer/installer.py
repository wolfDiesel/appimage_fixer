...
def install_systemd_service() -> None:
    ...
    try:
        ...
        subprocess.run(["systemctl", "--user", "enable", "appimage-fixer.timer"], check=True)
        subprocess.run(["systemctl", "--user", "start", "appimage-fixer.timer"], check=True)
...
def uninstall_systemd_service() -> None:
    ...
    try:
        subprocess.run(["systemctl", "--user", "stop", "appimage-fixer.timer"], 
                      check=False, capture_output=True)
        subprocess.run(["systemctl", "--user", "disable", "appimage-fixer.timer"], 
                      check=False, capture_output=True)
        subprocess.run(["systemctl", "--user", "daemon-reload"], check=True)
...
def check_service_status() -> dict:
    ...
    try:
        ...
        service_result = subprocess.run(
            ["systemctl", "--user", "show", "appimage-fixer.service", "--property=ExecMainStatus"],
            capture_output=True, text=True
        )
        
        return {
            "timer_active": timer_active,
            "service_installed": (get_systemd_user_dir() / "appimage-fixer.service").exists(),
            "timer_installed": (get_systemd_user_dir() / "appimage-fixer.timer").exists()
        }
